...
| `MONGO_USER` / `MONGO_PASSWORD` | `tasksense` / `openssl rand -hex 24` | **Bundled database only** — with `MONGODB_URI` set they are unused and stay empty. The database account. **Hex, not base64.** The password is substituted into a connection string, and a URI reads `/ : @ ? # [ ] %` as structure — one of those ends the password early and the driver rejects the whole URI with *Password contains unescaped characters*, before connecting to anything. |
